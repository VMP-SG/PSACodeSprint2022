...
function DropZone({ onDrop, accept, open, image, thumbnail, status }) {
  const dropzoneColour =
    status === 3 ? "green-0" : status === 2 ? "dark-red" : "blue-link";
    ...
    <div>
        ...
        <div {...getRootProps({ className: "dropzone" })}>
          ...
          <div className="text-center py-16 border-2 border-blue-link rounded-2xl h-[140px]">
            {isDragActive ? (
              ...
              <Ctext
                styles={
                  "flex justify-center items-center text-subtitle text-blue-link font-semibold cursor-pointer h-full"
                }
              >
      ...
      {acceptedFiles.length > 0 && !thumbnail && (
        <div>
          ...
          <Ctext styles={"text-card-body text-blue-link"}>
            Uploaded {acceptedFiles[0].path}
          </Ctext>
        </div>
      )}
      {thumbnail && !image ? (
